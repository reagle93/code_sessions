...
class NAryTree
{
	...
	void getMinIterUtil(int v, vector<int>& minItr);
public:
	NAryTree(int N); 
...
void NAryTree::getMinIterUtil(int u, vector<int>& minItr)
{
	minItr[u] = adj[u].size();
  vector<int> minItrChild(minItr[u], 0);

	for (auto i = adj[u].begin(); i!= adj[u].end(); ++i)
	{
		getMinIterUtil(*i, minItr);
		minItrChild.push_back(minItr[*i]);
	}
  
  sort(minItrChild.begin(), minItrChild.end(), greater<int>());
  
	for (int j = 0; j < adj[u].size(); j++)
	{
		minItr[u] = max(minItr[u], minItrChild[j] + j + 1);
	}
}

int NAryTree::getMinIter()
{
    vector<int> minItr(N, 0);
	int res = -1;
